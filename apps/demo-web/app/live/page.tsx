...
type MentoringSummary = {
    mentoringId: number;
    isGroup: boolean;
    status: 'READY' | 'ON_AIR' | 'COMPLETED';
};
...
export default function LiveMentoringPage() {
    ...
    const [mentoringMode, setMentoringMode] = useState<'oneToOne' | 'group'>('group');
    ...
    const connectWebSocket = useCallback(
        async (mentoringId: number, selectedRole: 'mentor' | 'mentee', userId: number, isGroup: boolean) => {
            ...
            socket.on('signal', (message: RpcMessage) => {
                try {
                    ...
                    if (message.event === 'new-producer') {
                        ...
                        if (!isGroup) {
                            if (producerData.kind && producerData.kind !== 'audio') {
                                return;
                            }
            ...
            if (selectedRole === 'mentor' || !isGroup) {
                ...
                const stream = await navigator.mediaDevices.getUserMedia({
                    video: selectedRole === 'mentor' && isGroup,
                    audio: true
                });
                ...
                if (videoTrack && selectedRole === 'mentor' && isGroup) {
            ...
            if (selectedRole === 'mentee' || !isGroup) {
                ...
                const producerIdsToConsume = joinResponse.existingProducerIds
                    .filter((item) => {
                        if (!isGroup) {
                            return item.kind === 'audio';
                        }

                        return selectedRole === 'mentee' && item.role === 'mentor';
                    })
    ...
    const fetchMentoringSummary = useCallback(async (mentoringId: number) => {
        const response = await fetch(`${HTTP_BASE}/mentorings/${mentoringId}`);

        if (!response.ok) {
            throw new Error(`멘토링 조회 실패 (${response.status})`);
        }

        const payload = (await response.json()) as { mentoring: MentoringSummary };
        return payload.mentoring;
    }, []);
    ...
    const handleMentorStart = useCallback(async () => {
        ...
        try {
            ...
            const isGroup = mentoringMode === 'group';

            setStatus('멘토로 접속 중...');
            await connectWebSocket(mentoringId, 'mentor', userId, isGroup);
            setRole('mentor');
            setStatus(`멘토 접속 완료 (${isGroup ? '1:N' : '1:1'} / 멘토링 ID: ${mentoringId})`);
        } catch (error) {
            setStatus('오류 발생');
            setErrorMessage(error instanceof Error ? error.message : '알 수 없는 오류');
        }
    }, [connectWebSocket, mentoringMode, parseUserIdInput]);
    ...
    const handleMenteeJoin = useCallback(async () => {
        ...
        try {
            ...
            const mentoring = await fetchMentoringSummary(mentoringId);

            if (mentoring.status !== 'ON_AIR') {
                throw new Error('진행 중(ON_AIR)인 멘토링만 참여할 수 있습니다.');
            }

            setStatus('멘티 접속 중...');
            await connectWebSocket(mentoringId, 'mentee', userId, mentoring.isGroup);
            setRole('mentee');
            setStatus(`멘티 접속 완료 (${mentoring.isGroup ? '1:N' : '1:1'} / 멘토링 ID: ${mentoringId})`);
        } catch (error) {
            setStatus('오류 발생');
            setErrorMessage(error instanceof Error ? error.message : '알 수 없는 오류');
        }
    }, [connectWebSocket, fetchMentoringSummary, mentoringIdInput, parseUserIdInput]);
                    ...
                    <div className="space-y-3">
                        ...
                        <label className="block text-sm">
                            ...
                            <select
                                className="mt-1 w-full rounded-md border border-zinc-700 bg-zinc-950 px-3 py-2"
                                value={mentoringMode}
                                onChange={(event) => setMentoringMode(event.target.value as 'oneToOne' | 'group')}
                            >
                                <option value="oneToOne">1:1 (isGroup=false)</option>
                                <option value="group">1:N (isGroup=true)</option>
                            </select>
                        </label>
                        <button
                            type="button"
